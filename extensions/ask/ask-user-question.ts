...
/** Standard terminal bell — same byte the terminal-warp transport uses as OSC_TERMINATOR. */
export const BEL = "\x07";

/**
 * Emit one portable terminal attention signal without touching redirected output.
 * Writes to stdout rather than the terminal-warp transport's `/dev/tty`: the `isTTY` gate
...
/** Delay before the background session-graph pre-warm; mirrors the workflow extension's /wf prewarm. */
...
export const DEFAULT_PROMPT_GUIDELINES: string[] = [
	`ask_user_question: use when blocked on concrete user decisions; one invocation per need — never stack calls back-to-back.`,
	`ask_user_question: put a recommendation first with "(Recommended)"; never author "Other"/"Type something" rows (reserved labels are rejected at runtime).`,
];
