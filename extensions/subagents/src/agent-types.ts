 ...
 * to exactly one enabled agent. Default is fail-closed (`NO_FALLBACK`): an
 * unresolvable type errors naming the available types instead of silently
 * inheriting max-privilege general-purpose. Set from `subagents.json`
 * (`fallbackSubagent`); an explicit name there still wins.
 *
 * Module state rather than an index.ts closure because every caller-supplied
 * dispatch path funnels through it.
 */
let fallbackSubagent: string | undefined = NO_FALLBACK;
/** Get the configured fallback agent type. Default is fail-closed (`none`). */
export function getFallbackSubagent(): string | undefined { return fallbackSubagent; }

/** Set the configured fallback agent type. `undefined` restores fail-closed default. */
export function setFallbackSubagent(v: string | undefined): void { fallbackSubagent = v ?? NO_FALLBACK; }
/**
...
export function resolveSpawnTypeIn(
  ...
  // Unset: fail-closed by default (module initialises to NO_FALLBACK), so a
  // deleted file AND a merely disabled one both error naming the available
  // types instead of quietly dropping to general-purpose.
  return { ok: false, message: `${reason} Available: ${available()}.` };
}
