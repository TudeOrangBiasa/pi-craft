...
async function runOneShellCommand(command: string, pi: ExtensionAPI, cwd: string, timeoutMs: number): Promise<string> {
	...
	if (res.code !== 0) {
		// FR2 budget preserved: formatShellOutput merges + truncates both streams
		// identically to the success path, so a failed command keeps its stdout
		// evidence (test runners print primary diagnostics there) within budget.
		return `[Shell error: exit code ${res.code}]\n${formatShellOutput(res)}`;
	}
 ...
 * `_expandSkillCommand` is skipped — `pi-args` is the only expander on that
