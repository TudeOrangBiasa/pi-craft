/**
 * vision — give text-only models vision by proxying image input
...
// Zero-risk hash memo: keyed by block object identity. Same object ⇒ same
// bytes, so a hit can never alias a different image. Blocks are fresh objects
// each turn, so cross-turn always recomputes full sha256; entries GC with
// their blocks, no cap needed.
const hashByBlock = new WeakMap<object, string>();
function blockImageHash(block: object, mimeType: string, data: string): string {
  const hit = hashByBlock.get(block);
  if (hit !== undefined) return hit;
  const hash = imageHash(mimeType, data);
  ...
  pi.on("context", async (event, ctx) => {
    ...
    for (const msg of messages) {
      ...
      for (const block of content) {
        ...
        if (!img) continue;
        anyImage = true;
        const hash = blockImageHash(block, img.mimeType, img.data);
    ...
    if (entries.length > 0) {
      ...
      const descArr = await Promise.all(entries.map(([, img]) => loader.loadDescription(img)));
      if (ctx.signal?.aborted) return;

      warnFailedImages(ctx, entries.map(([_, img]) => img), descArr, lastDescriberError ?? "unknown error");
      for (let i = 0; i < entries.length; i++) {
        descs.set(entries[i][0], descArr[i]);
      }
    }
