...
- `replace`/`insert`: use for file edits instead of bash sed/awk/perl.
- `replace`/`insert`: anchors come only from `read`, a post-edit diff, or `anchor_grep` — NEVER invent them. Re-read after each edit when you need fresh anchors.