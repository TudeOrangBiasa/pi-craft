...
export default function (pi: ExtensionAPI) {
	...
	const rebuild = (ctx: ExtensionContext) => {
		// A queued prediction targets the pre-rebuild session — drop it.
		cancelPending();
		try {
			...
			// Stale ctx proxy (e.g. session_compact racing runner disposal):
			// never raise from a history refresh; restart from empty.
			history = scanCacheHistory([]);
		}
	};
	...
	const schedulePrediction = (
		...
		pendingPredictionTimer = setTimeout(() => {
			...
			try {
				...
				// Stale/disposed ctx by fire time: a prediction is advisory,
				// never worth an uncaught timer exception.
			}
		}, 0);
	};
