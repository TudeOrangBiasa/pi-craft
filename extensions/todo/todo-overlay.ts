...
export class TodoOverlay {
	...
	private renderWidget(theme: Theme, width: number): string[] {
		...
		// Only tasks actually rendered this frame enter the hide queue —
		// overflow-hidden completed must survive until truly shown once.
		...
		if (layout.truncatedTail > 0) {
			const pendingCount = layout.truncatedTail - layout.truncatedInProgress;
			if (pendingCount > 0) overflowParts.push(`${pendingCount} ${formatStatusLabel("pending")}`);
			if (layout.truncatedInProgress > 0) overflowParts.push(`${layout.truncatedInProgress} ${formatStatusLabel("in_progress")}`);
		}
