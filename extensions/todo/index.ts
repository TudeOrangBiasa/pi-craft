 ...
 *
 * TUI chrome strings resolve at render time via the i18n bridge, which uses
 * canonical-English fallbacks. This fork bundles no external i18n SDK, so
 * `t(key, fallback)` returns the inline English literal at every call site.
 *
 * Extracted from rpiv-pi@7525a5d. Tool name "todo" and widget key "pi-todos"
 * (renamed from upstream "rpiv-todos"; pre-fork session history does not replay).
 */
...
import { reminderText, shouldRemind } from "./src/reminder.js";
...
export default function (pi: ExtensionAPI, importOverlay: TodoOverlayImporter = () => import("./todo-overlay.js")) {
	...
	// TodoReminder: event-based nudge, no timers. Counts turns, stamps the
	// latest turn with todo-tool activity, injects one reminder message when
	// a full turn passed without a touch. Silent otherwise.
	...
	pi.on("tool_execution_end", async (event) => {
		if (event.toolName === TOOL_NAME && !event.isError) lastTodoTurn = completedTurns;
	});
	pi.on("before_agent_start", async () => {
		let open: Array<{ id: number; subject: string }> = [];
		try {
			open = getRenderState().tasks
				.filter((t) => t.status !== "deleted" && t.status !== "completed")
				.map((t) => ({ id: t.id, subject: t.subject }));
		} catch {
			return undefined;
		}
		if (!shouldRemind(open.length, completedTurns, lastTodoTurn)) return undefined;
		return {
			message: {
				customType: "todo-reminder",
				content: reminderText(open),
				display: false,
			},
		};
	});
