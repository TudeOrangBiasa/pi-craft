...
| --- | --- |
| unset, empty, or whitespace-only | `~/.config/pi-advisor/advisor.json` |
| absolute path | `$XDG_CONFIG_HOME/pi-advisor/advisor.json` |
| `~` or `~/…` | tilde expanded, then used as the config dir |
| relative path | ignored → `~/.config/pi-advisor/advisor.json` |
| `~user/…` | not expanded → `~/.config/pi-advisor/advisor.json` |
...
back once to `~/.config/pi-advisor/advisor.json` (always `~/.config`, ignoring
