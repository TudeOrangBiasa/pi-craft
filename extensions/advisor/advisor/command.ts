...
import { modelKey } from "./config-io.js";
...
// Mirror: the upstream models-extension items.ts — this picker is kept
// structurally in sync with it; evolve model-picker semantics together.
...
// Mirror: the upstream models-extension items.ts — same filter-in/map-out
// shape, different sentinels and persist target; evolve level-picker semantics together.
function buildEffortItems(picked: Model<Api>): SelectItem[] {
	...
	return [
		// "off (no reasoning sent)" ≠ the models extension's "off (disable reasoning)":
		// this row sends NO reasoning option; the models extension persists thinking:"off".
		...
		handler: async (_args, ctx) => {
			...
			if (picked.reasoning) {
				...
				if (!effortResult) {
					...
					// with the upstream models-extension stepper). The divergence is deliberate:
