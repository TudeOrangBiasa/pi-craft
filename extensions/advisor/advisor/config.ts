/**
 * config — persisted advisor config (~/.config/pi-advisor/advisor.json) and
 ...
 * lives in ./config-io.ts.
 */

import type { GuidanceFields } from "./config-io.js";
import { ADVISOR_CONFIG_PATH, loadJsonConfig, saveJsonConfig } from "./config-io.js";
...
export function validateDisabledForModels(value: unknown): DisabledForModelsEntry[] {
	...
	return value.filter((entry): entry is DisabledForModelsEntry => {
		...
		if (obj.minEffort !== undefined && !EFFORT_ORDINAL.includes(obj.minEffort as GradedEffort)) {
			...
			console.warn(
			`[pi-advisor] advisor.json: unknown minEffort "${String(obj.minEffort)}" — dropping disabledForModels entry for "${obj.model}"`,
			);
			return false;
		}
		return true;
