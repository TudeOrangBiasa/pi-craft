...
Let the model you're working with hand its whole conversation to a second, stronger model and get back a plan, a correction, or a stop signal — then keep going. `pi-advisor` adds the zero-parameter `advisor` tool and the `/advisor` picker to [Pi Agent](https://github.com/badlogic/pi-mono), so you can drive a session with a fast model and keep a stronger reviewer one call away.
...
The `/advisor` picker shows every model Pi already has credentials for, with the current selection highlighted.
...
Settings live in `~/.config/pi-advisor/advisor.json` (or
`$XDG_CONFIG_HOME/pi-advisor/advisor.json` when that variable is set to an
...
- [Configuration](./docs/configuration.md) — config file resolution, every key, blocklist matching rules, guidance overrides, and the full notification catalogue.
- [`advisor` tool reference](./docs/tool-reference.md) — schema, result envelope, failure paths, what gets sent to the reviewer, lifecycle hooks, and picker keys.
